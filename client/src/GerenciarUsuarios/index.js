...
export default withRouter((props) => {
  ...
  const handleSincronizarUsuarios = (event, rowData) => {
    setOpenSincronizaDialog({
      ...
      msg:
        "Deseja sincronizar as informações dos usuários com o serviço de autenticação?",
    ...
    if (confirm) {
      try {
        ...
        if (success) {
          ...
          setSnackbar({
            status: "success",
            msg: "Usuários sincronizados com sucesso.",
            date: new Date(),
          });
        }
      } catch (err) {
  ...
  const executeAdicionaDialog = useMemo(
    () => (status, msg) => {
      setOpenAdicionaDialog({});
      setRefresh(new Date());
      if (status && msg) {
    ...
    <>
      <MaterialTable
        ...
        columns={[
          ...
          {
            title: "Posto/Graducao",
            field: "tipo_posto_grad",
            editable: "never",
          },
